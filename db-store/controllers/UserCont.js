...
class UserCont {
  ...
  async findUser(req, res) {
    ...
    try {
      const result = await registerSchema.find({ _id: id });
      console.log(result);
  ...
  //   try {
  //     const result = await dirspermentSchems.find({ status: true ,id: req.User.id });
  ...
  //   try {
  //     const result = await salesSchema.find({ status: true, id: req.User.id });
  //     if (!result) {
  //       res.json(404, {
  //         status: "error",
